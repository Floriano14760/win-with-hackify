...
import { Eye, Zap, TrendingUp, CheckCircle2, Award } from 'lucide-react';
const advantages = [{
  icon: Eye,
      ...
      <div className="section-container" ref={ref}>
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <p className={`body-lg transition-all duration-700 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}> HACKIFY est un partenaire stratégique qui comprend les enjeux business et transforme chaque appel d'offres en opportunité de croissance.</p>
          </div>

          {/* Right grid */}
          <div className="space-y-4">
            {advantages.map((advantage, index) => <div key={advantage.title} className={`flex items-start gap-4 p-4 rounded-xl border border-transparent
                           hover:bg-card hover:border-border/50 transition-all duration-300
                           ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`} style={{
